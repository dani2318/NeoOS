...
def obj_path(src, subdir, build_dir):
    return os.path.join(build_dir, "kernel", subdir, src.lstrip("./") + ".obj")

def write_stage1_subninja(build_dir, asm):
...
rule asm_bin
  command = $asm -I src/boot/stage1 src/boot/stage1/nbootloader.asm -f bin -o $out && echo "--> Created stage1.bin"
...
rule asm
    command = mkdir -p {build_dir} && nasm {asmflags} -o $out $in && echo "--> Compiled: $in"
    description = ASM $in
...
def main():
    ...
    write_stage1_subninja(bdir,asm)
