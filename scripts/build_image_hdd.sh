...
mkfs.fat -n "NOOS" $TARGET_PARTITION >/dev/null
...
# copy files
echo "Copying files to ${TARGET_PARTITION} (mounted on /tmp/noos)..."
mkdir -p /tmp/noos
mount ${TARGET_PARTITION} /tmp/noos
cp ${BUILD_DIR}/kernel.bin /tmp/noos
cp test.txt /tmp/noos
mkdir /tmp/noos/test
cp test.txt /tmp/noos/test
umount /tmp/noos
